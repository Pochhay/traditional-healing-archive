const FILTERS = [
  { id: "all", label: "All Taxa" },
  { id: "name", label: "Names / ឈ្មោះ" },
  { id: "family", label: "Family" },
  { id: "uses", label: "Uses / ការប្រើប្រាស់" },
  { id: "compounds", label: "Compounds" },
  { id: "location", label: "Habitat" },
];

const styles = {
  wrap: {
    backgroundColor: "#18221D", border: "1px solid #2D4A3E",
    padding: "16px 18px", borderRadius: 4, display: "flex", flexDirection: "column", gap: 12,
  },
  inputBox: { position: "relative", display: "flex", alignItems: "center" },
  icon: { position: "absolute", left: 14, color: "#A3D1B8", display: "flex" },
  input: {
    width: "100%", padding: "12px 16px 12px 44px", backgroundColor: "#07100C",
  ...
  pill: (active) => ({
    padding: "5px 11px", borderRadius: 4, fontSize: 12, fontWeight: active ? 600 : 400,
    cursor: "pointer", border: active ? "1px solid #B2D095" : "1px solid #2D4A3E",
    backgroundColor: active ? "#B2D095" : "#232C27",
    color: active ? "#0C1511" : "#C1C8C2",
  }),
        ...
        <div style={styles.pills}>
          {FILTERS.map((f) => (
            <button
              ...
              style={styles.pill(activeField === f.id)}
            >
              {f.label}
            </button>
          ))}
